...
case class GetQuery[A, T <: Table](query: Query[T], cols: T#Columns[A], ordering: Option[(T#Column[_], OrderDirection)]) {
	...
	def order(column: query.table.Column[_], direction: OrderDirection): GetQuery[A, T] = order((column, direction))
	def order(columnAndDirection: (query.table.Column[_], OrderDirection)): GetQuery[A, T] = GetQuery(query, cols, Some(columnAndDirection))
